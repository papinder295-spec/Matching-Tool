...
STATIC_GEO_STANDARDIZATION_MAP = {
    ...
    # India Variants
    r"\b(india|bharat|hindustan|bhartiya)\b": "india",
}
...
# [NEW] Load Country/City file patterns for standardization
def load_country_city_regex(file_path=COUNTRY_CITY_SOURCE):
    ...
    try:
        ...
        rules = []
        for _, row in df.iterrows():
            country = str(row["CanonicalCountry"]).strip().lower()
            pattern = str(row[regex_col]).strip()
            if country and country != "nan" and pattern and pattern != "-" and pattern.lower() != "nan":
                try:
                    compiled = re.compile(pattern, flags=re.IGNORECASE)
                    rules.append((compiled, country))
                except Exception:
                    pass
        return rules
    except Exception:
...
def make_clean_name_fn(entity_regex, suffix_map=None, mandatory_words=None, geo_rules=None, dynamic_geo_regex=None):
    ...
    def clean_name(name):
        ...
        # [NEW] Dynamic Geo Regex Standardization
        if dynamic_geo_regex:
            for rule_regex, standard_term in dynamic_geo_regex:
                cleaned = rule_regex.sub(standard_term, cleaned)
...
def make_clean_address_fn(geo_rules=None, dynamic_geo_regex=None):
    ...
    def clean_address(addr):
        ...
        # [NEW] Dynamic Geo Regex Standardization for Addresses
        if dynamic_geo_regex:
            for rule_regex, standard_term in dynamic_geo_regex:
                cleaned = rule_regex.sub(standard_term, cleaned)
...
class MatcherApp(tk.Tk):
    def __init__(self):
        ...
        if self.dynamic_geo_regex:
            self._log(f"Dynamic Geo Standardization rules loaded successfully from File 6: {len(self.dynamic_geo_regex)} rules.")
        else:
